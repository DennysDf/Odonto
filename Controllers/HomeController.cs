...
namespace Odonto.Controllers
{
    ...
    {
        public readonly DBOdontoContext _context;
        public IHttpContextAccessor _http { get; set; }

        public HomeController(DBOdontoContext context, IHttpContextAccessor http)
        {
            _context = context;
            _http = http;
        }
        ...
        public async Task<IActionResult> Index(LoginVM model)
        {
            ...
            var user = _context.Usuarios.Include(c => c.Consultorio).Where(c => c.Login.Equals(model.Username.ToLower()) && c.Senha == model.Password);
            ...
            else
            {
                ...
                return RedirectToAction("Index", "Home", new { area = "Odonto" });
            }
        ...
        public IActionResult ConfirmarEmail(LoginVM model)
        {
            ...
            msg.From = new MailAddress("postmaster@devroyale.com", "nao-responder@dataodonto.com");

            msg.Subject = "Redefinir senha.";
            msg.Body = $"Seu código de verificação é <b>{randomNumber}</b> <br>Use-o para redefinir sua senha no sistema de gestão odontologica.";
            msg.IsBodyHtml = true;
            smtp.Send(msg);
